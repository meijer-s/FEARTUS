...
write.table(
  data_wide,
  file      = file.path(out_dir, "data_hip_reext_sham_unr_CS.csv"),
  row.names = FALSE,
...
# Generate the data frame for SUBJECT and CS
...
write.table(
  ...
  file      = file.path(out_dir, "blocks_hip_reext_sham_unr_CS.csv"),
  row.names = FALSE,
  col.names = FALSE,
  sep       = ","
)
...
# Add subject identity columns (each subject has the same column for both CS rows)
for (i in 1:subjects) {
...
write.table(
  design_matrix_df,
  file      = file.path(out_dir, "design_hip_reext_sham_unr_CS.csv"),
  row.names = FALSE,
  col.names = FALSE,  # exclude column names
  sep       = ","
)
...
write.table(
  contrast_matrix_df,
  file      = file.path(out_dir, "contrast_hip_reext_sham_unr_CS.csv"),
  row.names = FALSE,
  col.names = FALSE,
  sep       = ","
)
