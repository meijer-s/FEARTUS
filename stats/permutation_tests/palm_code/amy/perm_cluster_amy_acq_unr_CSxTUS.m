%% =============================
%  PALM Analysis: Acquisition CS × TUS
%  =============================
...
palm_in  = fullfile(basepath, 'palm_files',   'amy', 'amy_acq_unr_CSxTUS');
palm_out = fullfile(basepath, 'palm_results', 'amy', 'amy_acq_unr_CSxTUS');
...
EB = csvread(fullfile(palm_in, 'blocks_amy_acq_unr_CSxTUS.csv'));
M  = csvread(fullfile(palm_in, 'design_amy_acq_unr_CSxTUS.csv'));
EB = palm_reindex(EB, 'fixleaves');
Ptree = palm_tree(EB, M);
palm_ptree2dot(Ptree, fullfile(palm_in, 'exchangeability_amy_acq_unr_CSxTUS.dot'));

%% --- Render exchangeability tree (circo) ---
dotfile = fullfile(palm_in, 'exchangeability_amy_acq_unr_CSxTUS.dot');
svgfile = fullfile(palm_in, 'ptree_amy_acq_unr_CSxTUS.svg');
...
%% --- Run PALM ---
palm( ...
  '-i',  fullfile(palm_in, 'data_amy_acq_unr_CSxTUS.csv'), ...
  '-d',  fullfile(palm_in, 'design_amy_acq_unr_CSxTUS.csv'), ...
  '-t',  fullfile(palm_in, 'contrast_amy_acq_unr_CSxTUS.csv'), ...
  '-eb', fullfile(palm_in, 'blocks_amy_acq_unr_CSxTUS.csv'), ...
  '-Cstat', 'mass', '-C', 1.28, '-T', '-tfce1D', '-tableasvolume', ...
  '-o',  fullfile(palm_out, 'PALM_amy_acq_unr_CSxTUS'));
