...
write.table(data_wide, 
            file = file.path(out_dir, "data_amy_acq_sham_unr_CS.csv"), 
            row.names = FALSE, 
...
write.table(exchangeability_file, 
            file = file.path(out_dir, "blocks_amy_acq_sham_unr_CS.csv"), 
            row.names = FALSE, 
            col.names = FALSE, 
            sep = ",")
...
for (i in 1:subjects) {
  design_matrix[i, i + 1] <- 1
  design_matrix[25 + i, i + 1] <- 1
}
...
write.table(design_matrix_df, 
            file = file.path(out_dir, "design_amy_acq_sham_unr_CS.csv"), 
            row.names = FALSE, 
            col.names = FALSE,  # This will exclude column names
            sep = ",")  # Ensure the separator is a comma
...
write.table(contrast_matrix_df, 
            file = file.path(out_dir, "contrast_amy_acq_sham_unr_CS.csv"), 
            row.names = FALSE, 
            col.names = FALSE,  # This will exclude column names
            sep = ",")  # Ensure the separator is a comma
