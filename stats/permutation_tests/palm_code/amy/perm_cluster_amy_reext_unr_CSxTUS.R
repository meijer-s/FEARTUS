...
data_wide <- rbind(
  ...
  amygdala_sham_safety_df_wide,
  amygdala_active_threat_df_wide,
  amygdala_active_safety_df_wide
)

# Write the data to a CSV file
write.table(
  data_wide,
  file      = file.path(out_dir, "data_amy_reext_unr_CSxTUS.csv"),
  row.names = FALSE,
...
write.table(
  ...
  file      = file.path(out_dir, "blocks_amy_reext_unr_CSxTUS.csv"),
  row.names = FALSE,
  col.names = FALSE,
  sep       = ","
)
...
write.table(
  design_matrix_df,
  file      = file.path(out_dir, "design_amy_reext_unr_CSxTUS.csv"),
  row.names = FALSE,
  col.names = FALSE,
  sep       = ","
)
...
write.table(
  contrast_matrix_df,
  file      = file.path(out_dir, "contrast_amy_reext_unr_CSxTUS.csv"),
  row.names = FALSE,
  col.names = FALSE,
  sep       = ","
)
