...
scr_acq_ext <- scr_df %>%
  filter(PHASE %in% c("acquisition", "retention", "extinction"),
         TUS == "active") %>% 
  mutate(
    PHASE = ifelse(PHASE == "acquisition", 1, ifelse(PHASE %in% c("retention", "extinction"), -1, NA)),
...
# Define data
N_SUB <-    as.numeric(length(unique(scr_acq_ext$SUBID)))  # Number of subjects
N_TRIAL <-  as.numeric(max(scr_acq_ext$TRIAL))             # Maximum number of trials per subject
Tsubj <-    rep(N_TRIAL, N_SUB)                            # Number of trials per subject

# SCR
scr_wide <- scr_acq_ext %>%
  arrange(SUBID, TRIAL) %>%
  group_by(SUBID) %>%
...
# PHASE
phase_wide <- scr_acq_ext %>%
  arrange(SUBID, TRIAL) %>%
  group_by(SUBID) %>% 
  mutate(row_id = row_number()) %>%
  ungroup() %>%
  select(row_id, SUBID, PHASE) %>%
  spread(key = SUBID, value = PHASE) %>%
  select(-(row_id))

# Create data list for Stan
data_RW_TUS_acq_ext <- list(
...
transformed parameters {
  ...
  vector[N] TUS;                       // TUS parameter per subject
  ...
  UR_intercept = mu_p[3] + error_p[3] * UR_intercept_pr;
  UR_slope = mu_p[4] + error_p[4] * UR_slope_pr;
  intercept = mu_p[5] + error_p[5] * intercept_pr;
  slope  = mu_p[6] + error_p[6] * slope_pr;
  error = exp(mu_p[7] + error_p[7] * error_pr);
...
fit_RW_TUS_acq_ext <- stan(
  model_code = model_RW_TUS_acq_ext,
  data = data_RW_TUS_acq_ext,
  iter = 2000,
  warmup = 200,
  chains = 4,
  cores = 4,
...
save_fit(fit_RW_TUS_acq_ext, "fit_RW_TUS_acq_ext")
# Load raw model fit
fit_RW_TUS_acq_ext <- read_fit("fit_RW_TUS_acq_ext")

# Compute summary fit
summary_fit <- summary(fit_RW_TUS_acq_ext, pars = c("LR", "TUS", "UR_intercept", "UR_slope", "intercept", "slope", "error"))
# Save summary fit
save_summ(summary_fit, "fit_RW_TUS_acq_ext")
# Load summary fit
summary_fit <- read_summ("fit_RW_TUS_acq_ext")
...
t_test_TUS <- t.test(TUS ~ Experiment, data = params_df, var.equal = TRUE)

# Print the results
print(t_test_LR)
print(t_test_TUS)

# Plot TUS values (amygdala-TUS vs. hippocampus-TUS)
params_df <- data.frame(
  Subject = rep(1:50, times = 1),
  Experiment = rep(c("Amygdala", "Hippocampus"), each = 25),
  TUS = TUS_values[1:50]
...
boxplot_TUS <- ggplot(data = params_df, aes(x = Experiment_jitter, y = TUS)) +
  ...
  theme_classic() +
  xlab("") + 
  ylab(expression(bold("Emotional learning state"))) +
  theme(
